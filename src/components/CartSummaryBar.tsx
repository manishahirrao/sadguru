...
export default function CartSummaryBar() {
    ...
    return (
      <div className="fixed bottom-0 left-0 right-0 z-30 border-t border-[#241712]/10 bg-[#F7EFDD] px-4 py-3 shadow-[0_-4px_16px_rgba(0,0,0,0.08)]">
        <p className="text-center text-sm text-[#241712]/60">
          Your cart is empty —{" "}
          <Link href="/menu" className="font-semibold text-[#C15B2C] underline-offset-2 hover:underline">
            browse the menu
          </Link>
        </p>
      </div>
  ...
  return (
    <>
      ...
      {/* ── Left-side drawer ────────────────────────────────────────────── */}
      {drawerOpen && (
        <div className="fixed inset-0 z-50 flex" role="dialog" aria-modal="true" aria-label="Cart" id="cart-drawer">
          ...
          <div className="relative flex h-full w-full max-w-sm flex-col bg-[#F7EFDD] shadow-2xl animate-slideInLeft">
            ...
            <div className="flex items-center justify-between border-b border-[#241712]/10 px-5 py-4">
              <h2 className="font-display text-lg font-semibold text-[#241712]">Your Order</h2>
              <button
            ...
            <ul className="flex-1 overflow-y-auto px-5 py-3">
              {items.map((item) => (
                <li key={item.id} className="flex items-center gap-3 border-b border-[#241712]/5 py-3 last:border-0">
                  <div className="flex-1 min-w-0">
                    <p className="truncate text-sm font-medium text-[#241712]">{item.name}</p>
                    {item.selectedVariant && (
                      <p className="text-xs text-[#241712]/50">{item.selectedVariant}</p>
                    )}
                  </div>
                  <div className="flex items-center gap-1.5">
                    <button onClick={() => updateQty(item.id, item.qty - 1)} className="flex h-7 w-7 items-center justify-center rounded-full border border-[#241712]/20 text-[#241712] hover:bg-[#241712] hover:text-white focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-[#C15B2C]" aria-label={`Remove one ${item.name}`}>–</button>
                    <span className="min-w-[1.25rem] text-center text-sm font-medium">{item.qty}</span>
                    <button onClick={() => updateQty(item.id, item.qty + 1)} className="flex h-7 w-7 items-center justify-center rounded-full border border-[#241712]/20 text-[#241712] hover:bg-[#241712] hover:text-white focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-[#C15B2C]" aria-label={`Add one more ${item.name}`}>+</button>
                  </div>
                  <span className="min-w-[3rem] text-right font-mono text-sm font-semibold text-[#D9A441]">₹{item.lineTotal}</span>
                </li>
              ))}
            </ul>
            ...
            <div className="border-t border-[#241712]/10 bg-[#F7EFDD] px-5 py-5">
              ...
              <div className="space-y-1.5 text-sm">
                ...
                <div className="flex justify-between text-[#241712]/70">
                  <span>Delivery</span>
                  <span>{deliveryCharge === 0 ? <span className="font-medium text-green-600">FREE</span> : `₹${deliveryCharge}`}</span>
                </div>
      ...
      <style>{`
        ...
        @media (prefers-reduced-motion: reduce) { .animate-slideInLeft { animation: none; } }
